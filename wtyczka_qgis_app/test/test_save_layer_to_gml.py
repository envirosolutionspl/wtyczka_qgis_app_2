...
class SaveLayerToGmlTest(unittest.TestCase):
    ...
    def test_save_layer_to_gml(self):
        ...
        # with  as tmpdir:
        s = QgsSettings()
        s.setValue('wtyczka_qgis_app/settings/defaultPath', tmpdir)
        s.setValue('wtyczka_qgis_app/settings/strefaPL2000', 2176)
        s.setValue('wtyczka_qgis_app/settings/rodzajZbioru', 'POG')
        s.setValue('wtyczka_qgis_app/settings/jpt', '321202')
        ...
        if not os.path.exists(granice_dst):
            shutil.copytree(granice_src, granice_dst, dirs_exist_ok=False)
