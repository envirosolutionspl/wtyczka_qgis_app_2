...
DATA_ROOT = Path(__file__).parent / "pog_test_layers"
...
def test_pog_workflow(dataset_dir: Path, tmp_path, monkeypatch):
    ...
    settings.setValue("qgis_app2/settings/strefaPL2000", "2180")
    settings.setValue("qgis_app2/settings/defaultPath", str(dataset_dir))

    # Provide jpt from POG layer attribute
    pog_layer_path = dataset_dir / "pog" / "AktPlanowaniaPrzestrzennego.gml"
    settings.setValue("qgis_app2/settings/jpt", extract_jpt_from_pog(pog_layer_path))
